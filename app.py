...
target = st.text_input("输入你要凑的目标 默认是600",value=600)
target = float(target)
...
def fun(prices,target):
    solver = pywraplp.Solver.CreateSolver("SCIP")
    x = {}
    for i in range(len(prices)):
        x[i] = solver.IntVar(0, 1, "x[%i]" % i)
    solver.Add(sum(x[i]*price for i,price in enumerate(prices)) >= target)
